...
# =============================================================================
# 3_burnEmmcRootfs.sh
# Formats the KR260 eMMC root partition (mmcblk0p2) as ext4 and extracts
# the Ubuntu rootfs tarball into it. Optionally installs kernel modules.
#
# When --ver is used, the rootfs tarball and modules tarball are located
# automatically by glob-matching inside the version directory (any *.tar.gz
# for rootfs, and any modules-*.tgz for kernel modules).
#
# Usage:
...
# ---------------------------------------------------------------------------
rootfsTarPath="./ubuntu-rootfs.tar.gz"
modulesTarPath=""
...
if [[ -n "${selectedVer}" ]]; then
    ...
    if [[ -f "${modulesGlob[0]}" ]]; then
        ...
        # Try alternative glob (petalinux sometimes uses a different suffix)
...
# ---------------------------------------------------------------------------
logBanner "KR260 eMMC Programmer – Step 3/3: Burn RootFS Partition${selectedVer:+ [${selectedVer}]}"

if [[ ! -f "${rootfsTarPath}" ]]; then
    logError "Ubuntu rootfs tarball not found at '${rootfsTarPath}'. Aborting."
    if [[ -z "${selectedVer}" ]]; then
        logWarn "Tip: use --ver <version> to load files from a version directory."
        logWarn "Example: $(basename "$0") --ver mustV3.1"
