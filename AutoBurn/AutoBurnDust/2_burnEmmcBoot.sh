...
#
# Examples:
#   ./2_burnEmmcBoot.sh --ver dustV1.1               # Full repartition + both files
#   ./2_burnEmmcBoot.sh --ver dustV1.1 --only image  # Replace image.ub only
#   ./2_burnEmmcBoot.sh --ver dustV1.1 --only bit    # Replace system.bit only
# =============================================================================
...
# eMMC device (DUST: eMMC is mmcblk1, SD card/boot is mmcblk0)
...
if [[ -z "\${onlyFile}" ]]; then
    ...
    # Zeros the MBR so BusyBox fdisk always sees a blank disk –
    # works on brand-new SOMs with no partition table at all.
    runSudo dd if=/dev/zero of="\${emmcDev}" bs=512 count=1 2>/dev/null

    echo "[board] Creating partitions (p1=1GiB FAT32, p2=rest Linux) ..."
